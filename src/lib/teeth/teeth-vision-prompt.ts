...
export function buildTeethVisionPrompt(catalog?: TeethCatalogRef): string {
  ...
  parts.push(`- **Kolor dopiero potem fasony — nie myl kolejności. NIE przesuwaj koloru na drugi token.** Jeśli wiersz to "2B 01 15", to kolor = 2B, a fasony = 01, 15. Nigdy nie wybieraj "01" jako kolor tylko dlatego, że on też jest w katalogu Chromascop; jeśli token na pozycji koloru to "2B", to on właśnie jest kolorem.`);
  ...
  parts.push(`  - "ivo 2B 01 15 31 42" → sekcja Ivoclar, kolor 2B, fasony 01, 15, 31, 42 (przody, productLine "ivoclar_ivostar").`);
  ...
  parts.push(`  - "W G1 21 42 43 41" → sekcja Wiedent Estetic, kolor G1, fasony 21, 42, 43, 41.`);
  parts.push(`  - "M A3 0/8 1/60 70N" → sekcja Major, kolor A3, fasony 0/8, 1/60, 70N.`);
  ...
  parts.push(`\n**R. Rozróżnianie linii po kolorach i fasonach na kartkach mieszanych.** Kartka może zawierać sekcje z różnych linii obok siebie. Nie wczytuj wszystkiego do jednej linii (np. do Wiedent). Ustal productLine na podstawie koloru i fasonów w sekcji:`);
  parts.push(`- Kolory typowe dla Ivostar Chromascop: "2B", "2A", "3E", "1C", "1A", "2C", "3C", "4D", "4A", "6B", "4B", "6C", "01", "1D", "1E", "3A", "5B", "2E", "6D", "4C" — zazwyczaj z fasonami 01-45 (przody) lub 80-88 (boki). productLine: "ivoclar_ivostar" lub "ivoclar_gnathostar".`);
  parts.push(`- Kolory Wiedent Estetic: "G1", "G2", "G3", "N2", "N3", "N5", "R1", "R3", "R5" oraz A1-A3/B2-B3 — z fasonami 2-cyfrowymi 12-50 (przody), 00-011 (dolne), 60-80 (boki). productLine: "wiedent_estetic".`);
  parts.push(`- Kolory Wiedent Classic/Almamiss: A1-D4 + OM; fasony Classic 3-4 cyfrowe (np. 402, 421, 635, 733), Almamiss 3-cyfrowe (np. 210, 320, 650, 790). productLine: "wiedent_classic" lub "wiedent_almamiss".`);
  parts.push(`- Major: kolory A1-D4 + 2C-3R; fasony "0/8", "1/60", "70N", "B", "L", "M", "S", "A", "T", "7A", "19A". productLine: "major_super_lux", "major_composite" lub "major_dent".`);
  parts.push(`- Jeśli w sekcji widać prefix "ivo"/"W"/"M" — użyj go jako pierwsze podejście, ale dalej zweryfikuj kolor i fasony.`);
