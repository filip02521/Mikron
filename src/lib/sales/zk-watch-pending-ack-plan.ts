...
export async function rollbackZkWatchPendingAck(
  ...
  const { data: rowsRaw, error: fetchError } = await supabase
    .from("individual_orders")
    .select(
      `${salesCancelAckSelect(caps)}, source_zk_watch_id, source_zk_number, sales_client_kh_id, sales_client_name, is_teeth, informacja_stock_out_reorder, products, symbol, mikran_code, subiekt_tw_id`
    )
