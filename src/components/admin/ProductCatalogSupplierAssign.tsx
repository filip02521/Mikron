...
export function ProductCatalogSupplierAssign({
  ...
  const controls = (
    <div className={cn("flex gap-1.5", compact ? "items-center" : "flex-col gap-2 sm:flex-row sm:items-end")}>
      ...
      <Button
        ...
        className={cn(
          ...
          !row.topSupplier && "border-amber-300 bg-amber-50"
        )}
      >
        {row.topSupplier ? (compact ? "Zmień" : "Zmień mapowanie") : "Przypisz"}
      </Button>
    </div>
  );
